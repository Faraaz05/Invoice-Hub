...
export const useMarkPaid = () => {
  ...
  return useMutation({
    mutationFn: ({ invoiceId, paymentMethod, transactionId }: { 
      invoiceId: number; 
      paymentMethod: string; 
      transactionId?: string 
    }) => invoiceAPI.markAsPaid(invoiceId, paymentMethod, transactionId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['invoices'] });
    },
  });
...
export const useInvoiceActions = () => {
  ...
  return {
    ...
    approve: (invoiceId: number, comments?: string) => 
      approveMutation.mutateAsync({ invoiceId, comments }),
    reject: (invoiceId: number, comments: string) => 
      rejectMutation.mutateAsync({ invoiceId, comments }),
    markPaid: (invoiceId: number, paymentMethod: string, transactionId?: string) => 
      markPaidMutation.mutateAsync({ invoiceId, paymentMethod, transactionId }),
    isLoading: submitMutation.isPending || approveMutation.isPending || 
               rejectMutation.isPending || markPaidMutation.isPending,
  };
};
